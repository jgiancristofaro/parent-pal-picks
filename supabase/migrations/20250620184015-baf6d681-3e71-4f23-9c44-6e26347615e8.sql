...
-- Create a new migration to fix the get_connection_suggestions function
-- This completely replaces the function with proper column qualification
DROP FUNCTION IF EXISTS public.get_connection_suggestions(uuid, integer, integer);
...
BEGIN
  ...
  friends_of_friends AS (
    ...
    FROM user_follows uf_temp
    JOIN public.user_follows uf2 ON uf_temp.friend_id = uf2.follower_id
  ...
  IF v_results_count < p_page_size THEN
    ...
    WHERE p.is_community_leader = true
      ...
      AND NOT EXISTS (
        ...
        WHERE uf4.follower_id = p_user_id AND uf4.following_id = p.id
      )
      AND NOT EXISTS (
        SELECT 1 FROM temp_connection_suggestions tcs1
        WHERE tcs1.user_id = p.id
      )
  ...
  IF v_results_count < p_page_size AND v_user_city IS NOT NULL AND v_user_city != '' THEN
    ...
    WHERE ul.is_primary = true
      ...
      AND p.id != p_user_id
      AND NOT EXISTS (
        SELECT 1 FROM public.user_follows uf5
        WHERE uf5.follower_id = p_user_id AND uf5.following_id = p.id
      )
      AND NOT EXISTS (
        SELECT 1 FROM temp_connection_suggestions tcs2
        WHERE tcs2.user_id = p.id
      )
    ORDER BY p.full_name ASC
    LIMIT (p_page_size - v_results_count);
  ...
  IF v_results_count < p_page_size THEN
    ...
    LEFT JOIN public.follow_requests fr ON p.id = fr.requestee_id 
      ...
      AND fr.status = 'pending'
    LEFT JOIN (
      SELECT uf6.following_id, COUNT(*) as follower_count
      FROM public.user_follows uf6
      GROUP BY uf6.following_id
    ) fc ON p.id = fc.following_id
    WHERE p.is_suspended = false
      AND p.id != p_user_id
      AND NOT EXISTS (
        SELECT 1 FROM public.user_follows uf7
        WHERE uf7.follower_id = p_user_id AND uf7.following_id = p.id
      )
      AND NOT EXISTS (
        SELECT 1 FROM temp_connection_suggestions tcs3
        WHERE tcs3.user_id = p.id
      )
