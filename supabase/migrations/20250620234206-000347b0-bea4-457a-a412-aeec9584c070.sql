...
-- Update the get_onboarding_suggestions function to be more flexible during onboarding
-- This allows it to work with a passed user ID parameter and be less strict with auth.uid()
...
BEGIN
  -- Get the authenticated user ID (either from auth.uid() or passed parameter)
  v_authenticated_user_id := COALESCE(auth.uid(), p_user_id);
  
  -- For onboarding, we allow either authenticated users OR valid user ID parameter
  IF v_authenticated_user_id IS NULL THEN
    RAISE EXCEPTION 'Authentication required or valid user ID must be provided';
  END IF;

  -- Use the authenticated user ID for all queries
  ...
  WHERE ul.user_id = v_authenticated_user_id 
  ...
  WHERE p.is_community_leader = true
    ...
    AND p.id != v_authenticated_user_id  -- Exclude the user themselves
    AND NOT EXISTS (
      -- Exclude users already followed
      SELECT 1 FROM public.user_follows uf 
      WHERE uf.follower_id = v_authenticated_user_id 
        AND uf.following_id = p.id
    )
    AND NOT EXISTS (
      -- Exclude pending follow requests
      SELECT 1 FROM public.follow_requests fr 
      WHERE fr.requester_id = v_authenticated_user_id 
  ...
  IF v_results_count < p_limit THEN
    ...
    WHERE uf1.follower_id = v_authenticated_user_id
      AND uf2.following_id != v_authenticated_user_id  -- Exclude the user themselves
      AND p.is_suspended = false
      AND NOT EXISTS (
        -- Exclude users already followed
        SELECT 1 FROM public.user_follows uf3 
        WHERE uf3.follower_id = v_authenticated_user_id 
