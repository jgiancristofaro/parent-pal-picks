...
interface SuggestedParentsSectionProps {
  suggestions: SuggestedProfile[];
  isLoading: boolean;
  onFollowStatusChange?: () => void;
}

export const SuggestedParentsSection = ({ 
  suggestions, 
  isLoading, 
  onFollowStatusChange 
}: SuggestedParentsSectionProps) => {
  if (isLoading) {
    return (
      <Card className="mb-6">
        <CardHeader>
        ...
        <div className="space-y-3">
          {suggestions.map((profile) => (
            <div key={profile.id} className="relative">
              ...
              />
              {profile.mutual_connections_count && profile.mutual_connections_count > 0 && (
